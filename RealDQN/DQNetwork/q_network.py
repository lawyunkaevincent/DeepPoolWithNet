...
class TaxiFairQNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Dueling architecture separates V(s) and A(s,a) streams, mirroring
        # DuelingZoneCNNQNetwork from DeepPoolZone/zone_qnetwork.py.
        if use_dueling:
    ...
    def forward(
        ...
        # Pass mask so DuelingCandidateScorerMLP can centre advantage over
        # valid candidates only.  CandidateScorerMLP ignores the mask arg.
        raw_q = self.scorer(x, mask=mask)  # [B, C]
        ...
        for b in range(B):
            ...
            for g in unique_groups:
                ...
                # Detach group statistics so gradients flow only through
                # individual candidate scores, not through the shifting
                # normalization — this prevents the feedback loop that
                # destabilises training.
                g_mean = group_scores.mean().detach()
                g_max = group_scores.max().detach()
                ...
                # Add learned taxi-quality bias (detached stats as input
                # so the bias net doesn't create circular gradient paths)
