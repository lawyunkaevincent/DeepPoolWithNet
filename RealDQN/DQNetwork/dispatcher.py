...
_route_cache: dict[tuple[str, str, str], float] = {}
_route_cache_tick: float = -1.0          # sim-time when cache was last valid


def _route_cache_maybe_clear(now: float) -> None:
    """Clear the route cache once per simulation tick so travel times stay
    fresh (traffic conditions change), but within a single tick we reuse
    results.  A single decision's generate_candidates call happens within
    one tick, so every findRoute call inside it benefits from the cache."""
    global _route_cache, _route_cache_tick
    if now != _route_cache_tick:
        _route_cache.clear()
        _route_cache_tick = now
...
def _route_time(from_edge: str, to_edge: str, vtype: str) -> float:
    ...
    Results are cached within the current simulation tick."""
    if from_edge == to_edge:
        return 0.0
    key = (from_edge, to_edge, vtype)
    cached = _route_cache.get(key)
    if cached is not None:
        return cached
    try:
        route = traci.simulation.findRoute(from_edge, to_edge, vtype, routingMode=1)
        tt = route.travelTime if route.travelTime > 0 else float("inf")
    except Exception:
        tt = float("inf")
    _route_cache[key] = tt
    return tt
...
def _estimate_eta_chain(start_edge: str, stops: list[Stop], vtype: str, now: float) -> list[float]:
    ...
    Returns [] immediately if any leg is unreachable (inf travel time).
    Early exit mirrors DeepPoolZone zone_env._compute_route_etas().
    """
    etas: list[float] = []
    ...
    for stop in stops:
        ...
        if tt == float("inf"):
            return []   # unreachable leg — abort entire chain
        current_time += tt
...
def _two_phase_insertion(
    request: Request,
    plan: TaxiPlan,
    taxi_id: str,
    requests: dict[str, Request],
    request_lookup_by_res_id: dict[str, Request],
    now: float,
    vtype: str,
    max_ride_factor: float = 1.5,
    max_wait: float = 300.0,
) -> CandidateInsertion | None:
    """
    Two-phase insertion from DeepPool paper (zone_env._build_pooled_plan),
    adapted to return a full CandidateInsertion with violation metrics.

    Phase 1: try every valid pickup insertion position; keep the one that
             minimises total plan completion time.
    Phase 2: from that pickup position, try every dropoff position after it;
             keep the one that minimises total plan completion time.

    This is O(2N) route calls per taxi vs the old O(N²) double loop.
    Returns ONE CandidateInsertion (best by completion time) or None.
    """
    existing_stops = plan.stops
    n = len(existing_stops)

    if n >= MAX_SERVICE_SEQ:
        return None

    # Frozen prefix — same rule as original generate_candidates
    if n == 0:
        frozen_prefix = 0
    else:
        first_pu_idx = next(
            (i for i, s in enumerate(existing_stops) if s.stop_type == StopType.PICKUP),
            None,
        )
        frozen_prefix = (first_pu_idx + 1) if first_pu_idx is not None else 0

    # Passengers already inside the taxi (no remaining PICKUP stop)
    onboard_ids_in_stops = {s.request_id for s in existing_stops if s.stop_type == StopType.PICKUP}
    baseline_onboard = sum(
        1 for rid in plan.onboard_request_ids if rid not in onboard_ids_in_stops
    )

    req_max_wait = getattr(request, "max_wait", max_wait)
    req_max_ride = getattr(request, "max_ride_factor", max_ride_factor)

    # Baseline ETAs for existing stops (no new request inserted)
    orig_etas = _estimate_eta_chain(plan.current_edge, existing_stops, vtype, now)
    if existing_stops and not orig_etas:
        return None  # existing route already unreachable

    # Key by (request_id, stop_type) so it survives Stop object copying
    baseline_eta_map: dict[tuple[str, StopType], float] = {
        (s.request_id, s.stop_type): orig_etas[i]
        for i, s in enumerate(existing_stops)
    }

    # ── Phase 1: find best pickup insertion position ─────────────────
    best_pu_idx: int = -1
    best_pu_stops: list[Stop] = []
    best_pu_etas: list[float] = []
    min_completion_p1: float = float("inf")

    for pu_idx in range(frozen_prefix, n + 1):
        # Capacity check at this insertion point
        occ = baseline_onboard
        for s in existing_stops[:pu_idx]:
            occ += 1 if s.stop_type == StopType.PICKUP else -1
        if occ + 1 > plan.capacity:
            continue

        pu_stop = Stop(StopType.PICKUP, request.request_id, request.person_id, request.from_edge)
        trial = (
            [Stop(s.stop_type, s.request_id, s.person_id, s.edge_id) for s in existing_stops[:pu_idx]]
            + [pu_stop]
            + [Stop(s.stop_type, s.request_id, s.person_id, s.edge_id) for s in existing_stops[pu_idx:]]
        )
        etas = _estimate_eta_chain(plan.current_edge, trial, vtype, now)
        if not etas:
            continue
        if etas[-1] < min_completion_p1:
            min_completion_p1 = etas[-1]
            best_pu_idx = pu_idx
            best_pu_stops = trial
            best_pu_etas = etas
    ...
    # ── Phase 2: find best dropoff position (must be after pickup) ───
    best_final_stops: list[Stop] = []
    best_final_etas: list[float] = []
    min_completion_p2: float = float("inf")

    for do_idx in range(best_pu_idx + 1, len(best_pu_stops) + 1):
        do_stop = Stop(StopType.DROPOFF, request.request_id, request.person_id, request.to_edge)
        trial = (
            [Stop(s.stop_type, s.request_id, s.person_id, s.edge_id) for s in best_pu_stops[:do_idx]]
            + [do_stop]
            + [Stop(s.stop_type, s.request_id, s.person_id, s.edge_id) for s in best_pu_stops[do_idx:]]
        )
        etas = _estimate_eta_chain(plan.current_edge, trial, vtype, now)
        if not etas:
            continue
        if etas[-1] < min_completion_p2:
            min_completion_p2 = etas[-1]
            best_final_stops = trial
            best_final_etas = etas

    if not best_final_stops:
        return None

    # Apply final ETAs onto Stop objects
    for stop, eta in zip(best_final_stops, best_final_etas):
        stop.eta = eta

    # Locate pickup/dropoff in final list
    pu_idx_f = next(
        (i for i, s in enumerate(best_final_stops)
    ...
    if pu_idx_f is None or do_idx_f is None:
        return None

    pu_eta = best_final_etas[pu_idx_f]
    do_eta = best_final_etas[do_idx_f]

    # ── Violation metrics (mirrors original generate_candidates loop) ─
    new_wait = max(0.0, pu_eta - request.request_time)
    new_wait_violation = max(0.0, new_wait - req_max_wait)

    actual_ride = do_eta - pu_eta
    allowed_ride = (
        req_max_ride * request.direct_travel_time
        if request.direct_travel_time > 0 else float("inf")
    )
    new_ride_violation = max(0.0, actual_ride - allowed_ride)

    max_existing_delay = 0.0
    total_existing_delay = 0.0
    max_pickup_delay = 0.0
    existing_count = 0
    exist_wait_viol_sum = 0.0
    exist_wait_viol_max = 0.0
    exist_ride_viol_sum = 0.0
    exist_ride_viol_max = 0.0

    for i, s in enumerate(best_final_stops):
        if s.request_id == request.request_id:
            continue
        orig_eta = baseline_eta_map.get((s.request_id, s.stop_type))
        if orig_eta is None:
            continue
        delay = max(0.0, best_final_etas[i] - orig_eta)
        r = request_lookup_by_res_id.get(s.request_id)
        if r is not None:
            if s.stop_type == StopType.PICKUP:
                rmw = getattr(r, "max_wait", max_wait)
                pw = best_final_etas[i] - r.request_time
                wv = max(0.0, pw - rmw)
                exist_wait_viol_sum += wv
                exist_wait_viol_max = max(exist_wait_viol_max, wv)
                max_pickup_delay = max(max_pickup_delay, pw)
            elif s.stop_type == StopType.DROPOFF and r.direct_travel_time > 0:
                max_allowed_ride = getattr(r, "max_ride_factor", max_ride_factor) * r.direct_travel_time
                pu_new_eta = next(
                    (best_final_etas[j] for j, ss in enumerate(best_final_stops)
                     if ss.request_id == s.request_id and ss.stop_type == StopType.PICKUP),
                    None,
                )
                if pu_new_eta is not None:
                    rv = max(0.0, (best_final_etas[i] - pu_new_eta) - max_allowed_ride)
                elif getattr(r, "pickup_time", None) is not None:
                    rv = max(0.0, (best_final_etas[i] - r.pickup_time) - max_allowed_ride)
                else:
                    rv = max(0.0, best_final_etas[i] - orig_eta)
                exist_ride_viol_sum += rv
                exist_ride_viol_max = max(exist_ride_viol_max, rv)
        max_existing_delay = max(max_existing_delay, delay)
        total_existing_delay += delay
        existing_count += 1

    avg_existing_delay = total_existing_delay / existing_count if existing_count else 0.0

    added_route_time = _compute_added_existing_passenger_ride_time(
        plan=plan,
        new_stops=best_final_stops,
        orig_etas=orig_etas,
        new_etas=best_final_etas,
        now=now,
    )

    return CandidateInsertion(
        request_id=request.request_id,
        taxi_id=taxi_id,
        pickup_index=pu_idx_f,
        dropoff_index=do_idx_f,
        resulting_stops=best_final_stops,
        added_route_time=added_route_time,
        pickup_eta_new=pu_eta,
        dropoff_eta_new=do_eta,
        ...
        existing_wait_violation_sum=exist_wait_viol_sum,
        existing_wait_violation_max=exist_wait_viol_max,
        existing_ride_violation_sum=exist_ride_viol_sum,
        existing_ride_violation_max=exist_ride_viol_max,
        is_feasible=True,
    )
...
def generate_candidates(
    ...
    """
    For the given request, generate one CandidateInsertion per eligible taxi
    using the two-phase insertion algorithm (DeepPool paper), plus a DEFER
    pseudo-action.

    Speed improvements vs the original O(N²) double-loop approach:
      1. SUMO route-time pruning: rank all eligible taxis by their travel time
         to the request pickup edge (one findRoute call each) and keep only the
         TOP_K_TAXIS closest.  Taxis that are physically too far to matter are
         skipped entirely — no insertion computation at all.
      2. Two-phase insertion: for each kept taxi, Phase 1 finds the best pickup
         position (O(N) route calls), Phase 2 finds the best dropoff position
         (O(N) more).  Total O(2N) per taxi vs the old O(N²).

    Total findRoute calls per request:
      Old: num_taxis × O(N²)   (e.g. 20 taxis × 60 calls = 1200)
      New: num_taxis × 1 (filter) + TOP_K_TAXIS × O(2N)  (e.g. 20+112 = 132)
    """
    NEAREST_FRACTION = 0.75  # consider only the 75% nearest taxis

    _route_cache_maybe_clear(now)
    ...
    # ── Step 1: rank taxis by SUMO route time to request pickup ──────
    # One findRoute call per taxi — much cheaper than the old O(N²) full
    # insertion enumeration done for every taxi.
    taxi_pickup_times: list[tuple[str, float, str]] = []  # (taxi_id, travel_time, vtype)
    for taxi_id, plan in taxi_plans.items():
        if taxi_id not in eligible_taxi_ids:
            continue
        if taxi_id not in active_vehicle_ids:
            continue
        if not plan.current_edge:
            continue
        try:
            vtype = traci.vehicle.getTypeID(taxi_id)
        except traci.TraCIException:
            continue
        tt = _route_time(plan.current_edge, request.from_edge, vtype)
        if tt == float("inf"):
            continue  # genuinely unreachable — skip
        taxi_pickup_times.append((taxi_id, tt, vtype))

    # Sort by travel time to pickup; keep only the nearest 75%
    taxi_pickup_times.sort(key=lambda x: x[1])
    top_k = max(1, int(len(taxi_pickup_times) * NEAREST_FRACTION))
    top_taxis = taxi_pickup_times[:top_k]

    # ── Step 2: two-phase insertion for each shortlisted taxi ─────────
    for taxi_id, _pickup_tt, vtype in top_taxis:
        plan = taxi_plans[taxi_id]
        candidate = _two_phase_insertion(
            request=request,
            plan=plan,
            taxi_id=taxi_id,
            requests=requests,
            request_lookup_by_res_id=request_lookup_by_res_id,
            now=now,
            vtype=vtype,
            max_ride_factor=max_ride_factor,
            max_wait=max_wait,
        )
...
def _refresh_taxi_plans(taxi_plans: dict[str, TaxiPlan]) -> None:
    ...
    try:
        ...
        raise  # connection is dead — let the caller handle episode termination
    for taxi_id, plan in taxi_plans.items():
...
class HeuristicDispatcher:
    ...
    def _termination_ready(self) -> bool:
        ...
        # min_expected counts running vehicles + persons still expected
        # (including those scheduled to depart later).  Taxis whose
        # initial trip is complete are NOT counted, so min_expected == 0
        # truly means no more persons will arrive.
        #
        # However, min_expected alone is not enough: we must also ensure
        # all tracked requests have been fully served.  And we must NOT
        # terminate before any requests have been tracked (min_expected
        # can momentarily be 0 between incremental route-file loads).
